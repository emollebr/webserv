...
/*   Updated: 2024/04/17 15:36:59 by jschott          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (begin < end) {
		if (*begin == "location"){
			...
			if (begin != end && *begin == "{" &&
				...
				ParseLocation *test = new ParseLocation(begin + 1, statementend - 1);
				std::cout << *test << std::endl;
				_locations[location_block.first] = new ParseLocation(begin + 1, statementend - 1);
				// _locations[*(begin - 1)] = new ParseLocation(begin + 1, statementend - 1);
...
void	ParseServer::parseServerDirective(std::deque<std::string>::iterator begin, std::deque<std::string>::iterator end){
	std::cout << "	Parsing Server Directive from " << *begin << " to " << *end << std::endl;
